...
public class Configuration_RoomList {
    ...
    public void getIterator(int roomId, String type){
        final var room = roomList.get(roomId);
        if (room == null) {
            throw new NoSuchRoomException(roomId);
        } else {
            if(type.equals("items")){
                for(Item temp : room.getItems()){
                    System.out.println(temp.getItemDescription());
                }
            }else{
                for(Creature temp : room.getCreatures()){
                    System.out.println(temp.getCreaturesDescription());
                }
            }
        }
    }
