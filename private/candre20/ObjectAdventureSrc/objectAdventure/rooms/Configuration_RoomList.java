...
public class Configuration_RoomList {
    ...
    public void getIterator(int ID, String type) throws NoSuchRoomException{
        final var rooms = roomList.get(ID);
        List<Item> itemHolder;
        List<Creature> creatureHolder;
        if (rooms == null){
            throw new NoSuchRoomException(ID);
        }
        else{
            if (type.equals("items")){
               itemHolder = rooms.getItems();
               for(int i = 0; i < itemHolder.size(); i++){
                   System.out.println(itemHolder.get(i).getItemDescription());
               }
            }
            else{
                creatureHolder = rooms.getCreatures();
                for(int i = 0; i < creatureHolder.size(); i++){
                   System.out.println(creatureHolder.get(i).getCreaturesDescription());
               }
            }
        }
    }
