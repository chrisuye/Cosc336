...
public class GameController {
    ...
    public String getCreatureDescriptions(Room theRoom){
        StringBuilder roomDescription = new StringBuilder();
        
        if (!theRoom.getCreatures().isEmpty()){
            theRoom.getCreatures().forEach(theCreature 
                    -> roomDescription.append(MessageFormat.format("\t{0}\n", theCreature.getCreaturesDescription()))
               );
        }
        else{
            roomDescription.append("\tNo nearby creatures.");
        }
        
        return roomDescription.toString();
    }
    
    //For non-specified room
    public String getCreatureDescriptions() {
        try {
            return getCreatureDescriptions(rooms.getRoom(getPlayer().getCurrentRoomId()));
        } catch (NoSuchRoomException e) {
            return "No creatures in a non-existant room.";
        }
    }
