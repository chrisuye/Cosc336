...
public class Configuration_RoomList {
    ...
    public void getIterator(String type){
        roomFactory factory = new roomFactory();
        if(type.equalsIgnoreCase("ITEM")){
            System.out.println("All items in all rooms:");
        }
        if(type.equalsIgnoreCase("CREATURE")){
            System.out.println("All creatures in all rooms:");
        }
        for (int i=0; i< roomList.size();i++){
            System.out.println(factory.GetRoomsStuff(type, getRoom(i)));
